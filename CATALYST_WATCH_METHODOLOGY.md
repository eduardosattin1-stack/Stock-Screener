...
4. **Inject (gated).** `git push origin ux-revamp:main` — a production deploy to the default branch, **intentionally gated**: it requires explicit approval each time (the manual checkpoint). Never force-push; verify it's a clean fast-forward first (`git rev-list --count ux-revamp..origin/main` must be `0`). **Since 2026-07-14 ALL scheduled routines are COMMIT-ONLY** (the `basket13-redebate` and `basket13-daily-mark` tasks previously carried ff-guarded auto-pushes; both stripped per the 2026-07-03 standing rule) — routines now emit a `push-ready: YES/NO` report line instead, and this manual push is the single deploy path for all git-published data (Catalyst Watch board + Basket 13).
