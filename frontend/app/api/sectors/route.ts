...
// 5D (week) / 1M (month) / ytd / 1Y per symbol (slow layer, long cache).
async function changeFor(item: Row): Promise<{ week: number | null; month: number | null; ytd: number | null; year: number | null }> {
  ...
  return { week: pick("5D"), month: pick("1M"), ytd: pick("ytd"), year: pick("1Y") };
}
...
export async function GET() {
  ...
  try {
    ...
    const [quoteRows, ratesRaw, gdpRaw, ...changes] = await Promise.all([
      ...
      ...universe.map((u) => changeFor(u).catch(() => ({ week: null, month: null, ytd: null, year: null }))),
    ]);
    ...
    const rows = universe.map((item, i) => {
      ...
      const ch = changes[i] || { week: null, month: null, ytd: null, year: null };
      return { name: item.name, symbol: item.symbol, accent: item.accent ?? null, price, day, week: ch.week, month: ch.month, ytd: ch.ytd, year: ch.year };
    });
