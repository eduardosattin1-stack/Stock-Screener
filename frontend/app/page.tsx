...
function StockRow({stock:s,expanded,onToggle,weights,rank}:{stock:StockData;expanded:boolean;onToggle:()=>void;weights:Record<string,number>;rank:number}){
  ...
  const prob=getProb(s.composite);
  return(
    <>
      <tr onClick={onToggle} style={{cursor:"pointer",borderBottom:"1px solid var(--border-subtle,#eef1ef)",transition:"background 0.12s",borderLeft:isTop20?"3px solid #2d7a4f":"3px solid transparent"}}
        ...
        <td style={{fontFamily:"var(--font-mono)",textAlign:"center",padding:"10px 6px",fontSize:11,fontWeight:700,color:prob.p10>60?"#10b981":prob.p10>40?"#d97706":"#ef4444"}}>{prob.p10}%</td>
        <td style={{fontFamily:"var(--font-mono)",textAlign:"right",padding:"10px 6px",fontSize:11}}><span style={{color:"#10b981",fontWeight:600}}>+{prob.gain}%</span><span style={{color:"var(--text-light,#9ca3af)",margin:"0 2px"}}>/</span><span style={{color:"#ef4444",fontWeight:600}}>{prob.dd}%</span></td>
