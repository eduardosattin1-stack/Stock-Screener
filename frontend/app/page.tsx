...
// METHODOLOGIES_CONFIG moved to app/methodologiesConfig.ts (shared with /basket/[key] pages)

// Annualized return off a base-100 NAV chain: (nav/100)^(365/days) − 1.
// Returns null under 14 days of live history — annualizing less is noise.
const annualizedPct = (t: any): string | null => {
  const nav = t?.nav;
  const inc = t?.inception_date;
  const hist = t?.history || [];
  const last = t?.last_date || hist[hist.length - 1]?.date;
  if (nav == null || nav <= 0 || !inc || !last) return null;
  const days = Math.round((new Date(last + "T00:00:00").getTime() - new Date(inc + "T00:00:00").getTime()) / 86400000);
  if (days < 14) return null;
  const ann = (Math.pow(nav / 100, 365 / days) - 1) * 100;
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 24 }}>
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                                ...
                                {/* The old double-click in-page detail view is superseded by the
                                    dedicated /basket/[key] pages (Bruno, 2026-07-20). */}
                                <h3
                                ...
                                >
                                  ...
                                  <a href={`/basket/${methShortKey(basket.path)}`} style={{ color: "inherit", textDecoration: "none" }}>{basket.name}</a>
                    ...
                    {(speculairBaskets.apex_tracking_weighted || speculairBaskets.apex_tracking) && (() => {
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 18, padding: "10px 14px", marginBottom: 14, borderRadius: 8, background: "var(--bg)", border: "1px solid var(--border)" }}>
                        <div>
                          ...
                          <div style={{ fontSize: 9, color: "var(--text-light)", fontFamily: "var(--font-mono)" }}>since {at.inception_date}{annualizedPct(at) ? <> · ann. <b>{annualizedPct(at)}</b></> : null}</div>
                        </div>
                        {(at.history || []).length > 1 && (() => {
                    ...
                    {(valueApex.value_tracking_weighted || valueApex.value_tracking) && (() => {
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 18, padding: "10px 14px", marginBottom: 14, borderRadius: 8, background: "var(--bg)", border: "1px solid var(--border)" }}>
                        <div>
                          ...
                          <div style={{ fontSize: 9, color: "var(--text-light)", fontFamily: "var(--font-mono)" }}>since {vt.inception_date}{annualizedPct(vt) ? <> · ann. <b>{annualizedPct(vt)}</b></> : null}</div>
                        </div>
                        {(vt.history || []).length > 1 && (() => {
                    ...
                    {(frApex.fr_tracking_weighted || frApex.fr_tracking) && (() => {
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 18, padding: "10px 14px", marginBottom: 14, borderRadius: 8, background: "var(--bg)", border: "1px solid var(--border)" }}>
                        <div>
                          ...
                          <div style={{ fontSize: 9, color: "var(--text-light)", fontFamily: "var(--font-mono)" }}>since {ft.inception_date}{annualizedPct(ft) ? <> · ann. <b>{annualizedPct(ft)}</b></> : null}</div>
                        </div>
                        {(ft.history || []).length > 1 && (() => {
