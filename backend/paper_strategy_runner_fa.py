...
Lifecycle:
  IF inception_date is None (first ever run):
    - Open whatever qualifies (could be 0-10)
    - Lock in spy_inception_price as today's SPY
    - Subsequent runs all measure vs this SPY baseline

  IF inception_date is set:
    - Standard rotation: close non-qualifying, open new qualifiers
    - Mark weekly even if basket is empty (basket_avg = 0%, alpha = -spy_return)

Output schema mirrors composite/momentum runners — frontend reuses
StrategyKPICard with no changes.

Usage:
  export FMP_API_KEY=...
  python3 paper_strategy_runner_fa.py [--dry-run]
"""
...
log = logging.getLogger("fa_runner")
...
def select_top_n(scan_data: dict) -> list[dict]:
    ...
    if picks:
        ...
        log.info("  Empty basket - no stocks pass FA gate this week")
    return picks
...
def open_first_basket(scan_date: str, picks: list[dict]) -> tuple[list, float]:
    """Returns (positions, spy_inception_price). Positions can be empty."""
    ...
    for p in picks:
        positions.append({
            ...
            "altman_z_at_entry": p.get("altman_z"),
...
def rotate_basket(scan_date: str,
    ...
    """Standard rotation. Note: in FA mode, a stock leaving the basket
    means EITHER it recovered past the gate (35% drawdown closed) or its
    fundamentals broke (Pio dropped, Altman dropped). Either way, it gets
    sold at current price. Recovery = win. Fundamentals breaking = exit at
    whatever loss. Bruno's spec: weekly rotation."""
    ...
    if to_remove:
        ...
        for p in to_remove:
            ...
            removed_log.append({
                ...
                # Reason: stock no longer passes FA gate. We don't know
                # whether it recovered or broke without re-running the gate
                # here. Score sign at exit gives a rough hint via return.
                "reason": "no_longer_qualifies",
            })
            log.info(f"  ROTATED OUT  {p['symbol']:<6} "
                     f"entry ${p['entry_price']:.2f} -> exit ${exit_price:.2f} "
                     f"({ret*100:+.2f}%, {days_held}d) - left FA gate")
    ...
    for p in to_add_picks:
        ...
        log.info(f"  ROTATED IN   {p['symbol']:<6} entry ${p['price']:.2f} - new FA qualifier")
...
def add_weekly_mark(history: dict, scan_date: str, position_returns: list[float]) -> Optional[dict]:
    """Always emit a weekly mark, even with empty basket. Empty basket
    contributes 0% return — alpha = -SPY_return for that week. Real outcome."""
    ...
    if position_returns:
        ...
        log.info(f"Weekly mark: EMPTY BASKET, SPY {spy_return:+.2f}%, "
                 f"alpha {mark['alpha_pp']:+.2f}pp (sat in cash), day {days_since}")
    return mark
...
def recompute_summary(history: dict) -> Optional[dict]:
    ...
    # FA-specific: weeks the basket sat empty (in cash)
    weeks_empty = sum(1 for m in weekly_marks if m.get("n_positions", 0) == 0)
...
def run(dry_run: bool = False):
    ...
    log.info(f"FALLEN ANGEL runner starting {today}")
    log.info(f"  Strategy: {STRATEGY_VERSION}, top-{TOP_N} by composite_fallen_angel, weekly rotation")
    ...
    # NOTE: empty picks is OK for FA — we still proceed.

    # FA-specific: use inception_date == None to detect first run, NOT empty
    # current_basket. The basket can be legitimately empty mid-run.
    is_first_run = history.get("inception_date") is None

    if is_first_run:
        log.info(f"First run - opening initial basket ({len(picks)} qualifiers)")
        positions, spy_inception = open_first_basket(scan_date, picks)
        if spy_inception <= 0:
            log.error("Could not fetch SPY inception price - aborting")
            return False
        ...
        if not positions:
            log.info(f"FA Day 1: empty basket, SPY locked at ${spy_inception:.2f}, "
                     f"will retry weekly")
    else:
        current = history.get("current_basket", [])
    ...
    snapshot_path = f"{BASKET_SNAPSHOT_PREFIX}{today}_fa.json"
    ...
    if history["current_basket"]:
        ...
        log.info(f"FINAL BASKET: EMPTY (SPY inception ${history.get('spy_inception_price', 0):.2f})")

    if history["summary"]:
        s = history["summary"]
        log.info(f"SUMMARY: {s['weeks_tracked']} weeks "
                 f"({s['weeks_empty']} empty), {s['n_rotations']} rotations, "
