...
def format_report_text(date, macro, top20, to_buy, to_sell, catalysts, options, erosion_alerts, held):
    ...
    # Top 20
    lines.append(f"\n  {'─'*75}")
    lines.append(f"  TOP 20 PORTFOLIO {'':>43}")
    lines.append(f"  {'#':>3} {'Sym':<8} {'Price':>8} {'Comp':>6} {'Signal':<12} "
                f"{'P(+10%)':>8} {'MaxGain':>8} {'MaxDD':>7} {'Action':<8}")
    lines.append(f"  {'─'*75}")
    ...
    for s in top20:
        ...
        lines.append(
            ...
            f"{s['expected_max_dd']:>+6.1f}% {status}{options_flag}{erosion_flag}"
        )
    ...
    lines.append(f"\n{'═'*80}")
    lines.append(f"  Generated by CB Screener v7 | screener.carbonbridge.nl")
    lines.append(f"{'═'*80}")
