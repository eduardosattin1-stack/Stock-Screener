...
#   pwsh -File C:\Users\Bruno\Stock-Screener\backend\opus_strategist.ps1
#
...
$picks = (Get-Content $inFile -Raw | ConvertFrom-Json).count
...
$system   = Get-Content $prompt -Raw
$data     = Get-Content $inFile -Raw
...
$combined | claude -p --model opus --output-format text | Out-File -FilePath $outFile -Encoding utf8
if ($LASTEXITCODE -ne 0) { throw "claude design step failed (exit $LASTEXITCODE)" }
if (-not (Test-Path $outFile) -or (Get-Item $outFile).Length -lt 2) { throw "empty strategy_output.json" }
