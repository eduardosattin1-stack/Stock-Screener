...
SYSTEM OF RECORD (decisive — read FIRST). The multi-agent DEBATE already ran on each name. When the debate conflicts with the raw screen factors, THE DEBATE WINS:
  ...
  - ROTATION DISCIPLINE (continuity — you are ACCOUNTABLE TO YOUR OWN PRIOR CALLS): FIRST read backend/_opus_debate/_director_ledger_disruptor.txt — your currently-HELD disruptors (entry + why) and every name you DROPPED in 2026. This is a LOW-CHURN winners book: KEEP a held disruptor UNLESS its thesis is BROKEN (a gross-margin / disruption-vector inflection, a lost flagship design, theme-demand rollover through thesis_break_px, a forensic/solvency flip, or it has FLIPPED to a disruptee) OR you have a STRICTLY-BETTER orthogonal beneficiary for that seat — say which. Do NOT churn a compounder on a marginally lower score. You MAY RE-ADD a previously-dropped name ONLY by citing a DOCUMENTED THESIS CHANGE since the drop date (new product/guidance/backlog, a materially lower price, a resolved overhang) — a better grade is NOT a thesis change; you may override but must OWN it in whats_changed. For EACH apex pick output decision / decision_rationale / whats_changed (see schema).
...
DISRUPTION-VECTOR SIGN GATE (DECISIVE — apply to EVERY name BEFORE the rubric; this book holds DISRUPTORS, not the DISRUPTED). Classify each name's relationship to the secular force acting on its theme as BENEFICIARY (the force GROWS its TAM / take-rate / content-per-unit — the disruption flows THROUGH it: the picks-and-shovels toll-taker, the share-gaining attacker) or DISRUPTEE (the force ERODES its pricing power, routes around it, or commoditizes its core on a FORWARD basis — it is the incumbent the disruption is aimed AT). Judge on EVIDENCE, not the (often benign) positive theme label: read the dossier's `true_competitors`, `gm_trajectory`, customer mix, and any "lost on price / free / freemium / AI-substitution / ARPU-compression" notes, then CROSS-CHECK the loser-theme vocabulary in backend/_opus_debate/secular_themes.json (ai-displacement, payments-disintermediation, linear-media-decline, autonomous-mobility, labor-arbitrage-deflation, reimbursement-compression, retail-channel-shift, energy-transition-loser) — if the name sits in a `victim_layer` of any loser theme (e.g. ai-displacement victim_layers include "legal/tax/DIY tooling" and "horizontal SaaS with thin switching costs"; payments-disintermediation includes "card-processing take-rate"), it is a PRESUMPTIVE DISRUPTEE on that axis regardless of how its positive theme is tagged.
  - A DISRUPTEE is INELIGIBLE — no matter how WIDE its current moat, how clean its TRAILING gross margin, or how CHEAP its multiple. In this universe those three are exactly what a soon-to-be-disrupted incumbent looks like: the cheap multiple is the market pricing the disruption OF this name; the wide moat is what it holds right up until the disruption finishes; the still-expanding GM is a LAGGING read that has not yet caught the forward repricing. A cheap incumbent in secular decline belongs in the VALUE book (as a value name under its secular_theme cap), NOT here. Set disruption_vector="disruptee", record it in the memo's vetoed list with its loser_theme, and DO NOT seat it.
  - Seat a name ONLY if you can affirmatively state in `vector_evidence` the MECHANISM by which the theme's secular force is a TAILWIND for it. If the sign is genuinely two-sided / contested, treat it as DISRUPTEE for ELIGIBILITY — do NOT give the benefit of the doubt; this is the highest-volatility sleeve and a wrong-sign name is the worst holding in it.
  - This is SEPARATE from pillars 2-3 (which score a name's strength WITHIN its paradigm): the sign gate asks whether the paradigm itself is moving TOWARD or AGAINST the name. A name can have a fortress moat in pillar 2 and still fail this gate.

RUBRIC — four pillars ~25 pts each, applied ONLY to names that clear ALL gates above INCLUDING the disruption-vector sign:
...
def disruptor_input():
    ...
    # Director rotation discipline: render the disruptor prior-decision ledger (held + 2026 drops).
    try:
        write_director_ledger("disruptor", DISRUPTOR_DIR / "apex_basket_disruptor.json", E.FRONTEND_DIR / "public" / "speculair_disruptor_tracking.json")
    except Exception as _e:
        print(f"WARN: disruptor ledger build failed ({_e})")
...
def disruptor_publish(push_gcs=False):
    ...
    picks = [p for p in apx.get("apex_basket", []) if isinstance(p, dict) and p.get("symbol")]
    try:                                              # capture this run's Director decisions into the year ledger
        append_decision_history("disruptor", apx)
    except Exception as _e:
        print(f"WARN: disruptor decision-history capture failed ({_e})")
