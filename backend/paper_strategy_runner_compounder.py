#!/usr/bin/env python3
"""
paper_strategy_runner_compounder.py
====================================
COMPOUNDER strategy (v8): top-20 SP500 stocks by compounder_score, equal-weight,
**monthly** rotation with 50% sector cap and Top-40 hysteresis exit.

Differences from the composite/momentum/FA runners:
  - TOP_N = 20 (not 10)
  - Monthly rebalance: rotates only if ≥28 days since last rotation date
  - 50% sector cap: max 10 of 20 stocks in any one sector
  - Top-40 hysteresis exit: a current position is dropped only if it falls
    out of Top 40 (or out of the qualified cohort entirely). Positions in
    Top 21–40 are kept — reduces churn vs strict Top-20 cutoff.
  - Sort key: compounder_score DESC, filter signal_compounder='DISQUALIFIED'

Lifecycle (runs weekly, but only rotates monthly):
  1. Read existing strategy_history_compounder.json from GCS
  2. Read latest_global.json from GCS
  3. Build qualified cohort sorted by compounder_score (top-20 + top-40 view)
  4. Mark-to-market every week.
  5. Rotate only if (first run) OR (≥28 days since last rotation_date):
     - Drop currently-held positions that fell out of Top 40 (or DISQUALIFIED)
     - Apply 50% sector cap when filling back to 20 positions from Top 20
  6. Append weekly_mark, recompute summary, write GCS.

Output schema mirrors paper_strategy_runner_composite.py so the frontend
Performance page can reuse StrategyKPICard with no changes (extra fields
sector / compounder_score_at_entry are additive).

Usage:
  export FMP_API_KEY=...
  python3 paper_strategy_runner_compounder.py [--dry-run] [--force-rotate]
"""
...
log = logging.getLogger("compounder_runner")

# ─────────────────────────────────────────────────────────────────────────
FMP_KEY = os.environ.get("FMP_API_KEY", "")
GCS_BUCKET = os.environ.get("GCS_BUCKET", "screener-signals-carbonbridge")

STRATEGY_VERSION = "compounder-v1.0-2026-05-10-global"
...
HISTORY_PATH = "performance/strategy_history_compounder.json"
...
def select_qualified_cohort(scan_data: dict) -> tuple[list[dict], dict[str, dict]]:
    """Filter scan to qualified Compounder cohort (signal_compounder='QUALIFIED'
    with a positive compounder_score and a usable price), sort by score DESC.
    ...
    candidates = []
    for s in stocks:
        if s.get("signal_compounder") != "QUALIFIED":
            continue
        score = s.get("compounder_score")
        if score is None:
            continue
        price = s.get("price")
        ...
        candidates.append({
            ...
            "compounder_rank": s.get("compounder_rank"),
            "price": float(price),
    ...
    log.info(f"Compounder cohort: {len(candidates)} qualified")
    if candidates:
...
def run(dry_run: bool = False, force_rotate: bool = False):
    ...
    log.info(f"COMPOUNDER runner starting {today}")
    log.info(f"  Strategy: {STRATEGY_VERSION}")
    log.info(f"  Top-{TOP_N} by compounder_score, monthly rotation "
    ...
    if not sorted_picks:
        log.error("No qualified Compounder picks — aborting")
        return False
    ...
    # 6. Snapshot
    snapshot_path = f"{BASKET_SNAPSHOT_PREFIX}{today}_compounder.json"
