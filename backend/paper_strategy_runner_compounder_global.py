#!/usr/bin/env python3
"""
paper_strategy_runner_compounder_global.py
===========================================
COMPOUNDER GLOBAL strategy (v8): top-20 GLOBAL stocks by compounder_score_global,
equal-weight, monthly rotation with 50% sector cap and Top-40 hysteresis exit.

Variant of paper_strategy_runner_compounder.py — same scoring logic and
lifecycle, but reads `compounder_score_global` / `signal_compounder_global`
fields instead of the SP500-only versions. Universe: every stock in the
scan with valid roe/pb/opmargin_delta and sector NOT in {Financial Services,
Insurance, Healthcare}. No SP500 membership filter.

Run alongside the SP500 Compounder. Two parallel paper-tracks let us
A/B test "does the SP500 filter add or destroy alpha?" live.

Caveats vs SP500 Compounder:
  - Currency exposure: basket may include JP, EU, HK names. Returns
    unhedged — mix stock alpha with FX moves.
  - Disclosure variability: operatingIncome convention differs across
...
Output: performance/strategy_history_compounder_global.json
        performance/baskets/{date}_compounder_global.json

Usage:
  export FMP_API_KEY=...
  python3 paper_strategy_runner_compounder_global.py [--dry-run] [--force-rotate]
"""
...
log = logging.getLogger("compounder_global_runner")
...
HISTORY_PATH = "performance/strategy_history_compounder_global.json"
...
def select_qualified_cohort(scan_data: dict) -> tuple[list[dict], dict[str, dict]]:
    """Filter scan to qualified GLOBAL Compounder cohort (signal_compounder_global=
    'QUALIFIED' with a positive compounder_score_global and a usable price),
    sort by score DESC.
    ...
    for s in stocks:
        if s.get("signal_compounder_global") != "QUALIFIED":
            continue
        score = s.get("compounder_score_global")
        if score is None:
            continue
        price = s.get("price")
        ...
        candidates.append({
            ...
            "compounder_rank": s.get("compounder_rank_global"),
            "price": float(price),
            "sector": s.get("sector") or "",
            "country": s.get("country") or "",  # global-only: track for FX awareness
    ...
    log.info(f"Compounder-Global cohort: {len(candidates)} qualified")
    if candidates:
        head = [(c["symbol"], round(c["compounder_score"], 3),
                 c["country"][:2], c["sector"][:12])
...
def rotate_basket(scan_date: str,
    ...
    if to_remove:
        ...
        for p in to_remove:
            ...
            log.info(f"  ROTATED OUT  {p['symbol']:<10} ({reason:<22}) "
    ...
    # `keep` items already have 'sector' / 'country' from prior open;
    # defensively pull from by_symbol if missing (legacy positions written
    # before these fields were tracked).
    for k in keep:
        if k["symbol"] in by_symbol:
            ref = by_symbol[k["symbol"]]
            if not k.get("sector"):
                k["sector"] = ref.get("sector", "")
            if not k.get("country"):
                k["country"] = ref.get("country", "")
    ...
    for entry in new_basket_meta:
        ...
        log.info(f"  ROTATED IN   {entry['symbol']:<10} entry ${entry['price']:.2f} "
                 f"(score {entry['compounder_score']:.3f}, "
                 f"{entry.get('country', '??')[:2]} / {entry.get('sector', '')[:14]})")
...
def run(dry_run: bool = False, force_rotate: bool = False):
    ...
    log.info(f"COMPOUNDER-GLOBAL runner starting {today}")
    log.info(f"  Strategy: {STRATEGY_VERSION}")
    log.info(f"  Top-{TOP_N} by compounder_score_global, monthly rotation "
    ...
    if not sorted_picks:
        log.error("No qualified Compounder-Global picks — aborting")
        return False
    ...
    # 6. Snapshot
    snapshot_path = f"{BASKET_SNAPSHOT_PREFIX}{today}_compounder_global.json"
    ...
    if history["current_basket"]:
        ...
        # Group by sector AND country for global-variant diagnostics
        ...
        for p in history["current_basket"]:
            ...
            log.info(f"  {p['symbol']:<10} ({ctry[:2]} / {sec[:14]:<14}) "
        ...
        log.info(f"  Country breakdown: {sorted(country_counts.items(), key=lambda x: -x[1])}")
    if history["summary"]:
        s = history["summary"]
