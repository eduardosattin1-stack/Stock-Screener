export const meta = {
  name: 'scaleout-45-debate',
  description: 'All-Opus multi-agent debate + adversarial Skeptic + Scale-Director over ~45 AI scale-out infrastructure names (optical / grid / thermal / power-semi / power-gen / memory); online-fetch, isolated from the weekly book; produces per-stock debate history + a scale-tier rating',
  phases: [
    { title: 'Debate', detail: '45 names x Interrogator/Architect/CRO, online-fetch', model: 'opus' },
    { title: 'Skeptic', detail: 'adversarial primary-source kill-check per name', model: 'opus' },
    { title: 'Scale Director', detail: 'cross-basket scale-tier assignment + correlation memo', model: 'opus' },
  ],
}
...
phase('Debate')
log('Scale-out pipeline: ' + NAMES.length + ' names across 6 layers (A optical · B grid · C thermal · D pwr-semi · E power-gen · F memory) → debate → skeptic → scale-director')
await pipeline(
  NAMES,
...
log('All debates + skeptics in. Running the Scale-Director cross-basket assignment over ' + SYMS.length + ' names.')
await agent(
...
log('Scale-out debate + skeptic + director complete.')
return 'DONE'
