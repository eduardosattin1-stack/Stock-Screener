export const meta = {
  name: 'mining-chain-map',
  description: 'Sonnet chain-mapping over the gated mining candidates (Radar-style, chunked; physical-anchor enforced)',
  phases: [{ title: 'ChainMap', model: 'sonnet' }],
}
const N = 8
phase('ChainMap')
await parallel(Array.from({ length: N }, (_, i) => () => agent(
  'You are the MINING CHAIN RADAR (chain-mapping + physical-anchor pass). Read backend/_opus_debate/mining_chains.json (the versioned chain taxonomy: ids, theses, value-chain layers, notes) and backend/_opus_debate/mining/_map_chunk_' + i + '.json (your candidate chunk: symbol/name/sector/industry/mcap + lane + Stage-B gates). This is the MINING book: producer and royalty_streamer names belong HERE; an equipment_services or utility name mapped into a mining chain is a split-rule error and is DROPPED at merge - map it with its true business_model anyway, never bend the label to keep a name. For EACH symbol decide, skeptically:\n' +
  ...
  'Write (Write tool) VALID JSON to backend/_opus_debate/mining/_chainmap_' + i + '.json as {"<SYM>": {physical_anchor, chains, business_model, commodity_revenue_share, value_chain_position, true_competitors, chain_fit_confidence}, ...} covering EVERY symbol in your chunk. Reply exactly: DONE',
  { label: 'miningmap:' + i, phase: 'ChainMap', model: 'sonnet' })))
return 'DONE'
