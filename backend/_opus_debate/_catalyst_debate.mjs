...
function skepticPrompt(n) {
  ...
  return 'You are the SPECULAIR SKEPTIC (Claude Opus 4.8) running an ADVERSARIAL kill-check on ' + sym + ' (' + n.co + '), an EVENT-DRIVEN special situation (' + n.label + ', ' + n.cluster + '). Default verdict REFUTED unless you can INDEPENDENTLY confirm the load-bearing facts against a PRIMARY source (SEC / 8-K, company IR, FDA / regulator pages, the merger proxy / agreement, an exchange notice). You see ONLY the bear side.\n' +
    ...
    '3. Verdict: CONFIRMED (the asymmetry survives) | CONFIRMED_WITH_CORRECTIONS (survives but a load-bearing fact / date needs fixing — state it) | REFUTED (the catalyst is fired / soft, the floor breaks, or the spread is gone). ALSO correction_severity: "minor" (footnote-level) or "material" (a load-bearing number/date/anchor moved). AND kill_scope: which layer your strongest finding hits - "thesis" | "numbers" | "catalyst" | "moat". If kill_scope is "numbers" you MUST ALSO emit revised_fv_px: your re-derived EVENT-WIN target per share (a point NUMBER in the same currency as the record) after applying your correction to the records own schedule, AND revised_floor_px: your re-derived downside floor per share. State the LEVELS - the pipeline computes every ratio and both implied probabilities. The post layer re-grades the line on YOUR numbers instead of treating a repricing as a thesis kill, so a numbers-REFUTED WITHOUT revised_fv_px is treated as a hard kill (fail-safe, never fail-open). Both are null for every other kill_scope. Do NOT emit any numeric conviction cap - verdicts and severity only.\n' +
    '3b. ANCHOR HONESTY (2026-08-04): when you refute a target as un-comparable (no comp supports it, the winner in the same category carries a lower EV, the consensus-delta pillar is contradicted by the tape), that is a NUMBERS kill and you must supply revised_fv_px. Do NOT leave the re-derived level in kill_fact prose only - a number that lives only in prose is discarded by every downstream consumer, and the refuted target stays the anchor. This is the exact failure the branch exists to stop.\n' +
    '4. Write (Write tool) VALID JSON to ' + SKEP + '/' + sym + '.json = {symbol(="' + sym + '"), verdict, kill_fact, corrections, correction_severity, kill_scope, revised_fv_px, revised_floor_px, evidence:[2-4 dated primary-source cites]}. Reply exactly: DONE'
}
...
await agent(
  ...
  '3. For EACH name assign a CONVICTION 0-100 on the SAME bands the apex book uses: 90-100 = table-pounding, maximal asymmetry, catalyst IMMINENT + huge live-to-target spread + hard floor; 70-89 = high-conviction aggressive entry (live dated catalyst, real spread, defined floor); 50-69 = solid but undated / soft or thinner floor; <50 = watchlist (no live date, thin spread, or shaky floor). Apply the Skeptic by VERDICT (never as a numeric ceiling): REFUTED forces <50; CONFIRMED_WITH_CORRECTIONS with correction_severity=material warrants a ~10-20pt haircut; minor corrections cost nothing. Be willing to score a genuinely asymmetric, imminent, well-floored setup 80+ — that is the question this run exists to answer; do NOT reflexively suppress it. Also would_seat(bool), posture(enter_now_carry | scale_in | wait_for_date | pass), expected_return_pct(to target from live), binding_reason(the single fact driving the score), catalyst_status, and copy cro_verdict + cro_conviction from the name\'s result file.\n' +
