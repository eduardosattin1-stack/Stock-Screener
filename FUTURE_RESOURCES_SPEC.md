# FUTURE RESOURCES — Build Spec (v1, 2026-07-02)
...
>
> **What this builds**: a hybrid two-lane basket — **"Future Resources"** — covering the physical-inputs
> bottleneck of every theme Speculair already tracks: AI capex needs power, the energy transition needs
> copper and uranium, defense tech needs strategic metals. Four value chains at launch: uranium/nuclear
> fuel cycle, copper/electrification, rare earths & strategic metals, power-for-AI. Lithium/battery is
> deliberately excluded until the oversupply clears (a regime tripwire, §4, decides when to revisit).
...
3. **Scope**: the four chains in `future_resources_chains.json` v1.0. US listings only
   (NYSE/NASDAQ/**AMEX** — the uranium/RE cohort lists on NYSE American). Both constraints are honest
   limits stated on the card: much copper/RE developer alpha lists on TSX/ASX and is out of scope v1.
...
Construction` are proven in-repo against this key (themes_map.py / disruptor_themes.json). `Copper`,
`Aluminum`, `Other Precious Metals & Mining`, `Specialty Chemicals` are **(verify)** — confirm against
`/stable/available-industries` on the first live run; the builder prints per-industry row counts so a
misspelled string shows up as a loud zero, never a silent gap.
...
Gold industry will catch non-chain royalty cos — both must drop loudly).
...
instances / §5 tripwires / change log). **Protocol**: 4 parallel research agents, one per chain, each
producing with citations: spot vs incentive price (incentive = cited feasibility/analyst consensus,
never invented), inventories, term-contracting state, futures-curve shape, COT positioning, policy
datelines. Verdict per chain: **TAILWIND / NEUTRAL / HEADWIND** + a tripwire table (including the
lithium re-entry tripwire: sustained spot > marginal-producer incentive price = revisit the exclusion).
...
stress**: all four chains share one "global growth + China demand" axis — decompose the final 8 on it
EVERY run, plus chain-specific axes (uranium: one utility contracting cycle; power-for-AI: one
hyperscaler's PPA appetite — the same axis the disruptor book stresses, flag cross-book).
...
without a docket date, PEA-stage economics presented as DFS, serial-diluter financing patterns.
...
dispatch wiring, and offline guard behavior — the live-key run is Bruno's first-run checklist, §9.1.)*
...
### 9.1 First-live-run checklist (Bruno, one-time — needs the FMP key)
...
2. `python backend/weekly_opus_refresh.py fr-universe` → funnel prints; spot-check CCJ/UEC/UUUU (AMEX
   canary), FCX/SCCO, MP, GEV/CEG present; TSLA/JPM-class absent.
